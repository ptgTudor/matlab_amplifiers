function summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4)

f1 = 100; % frequency
f2 = 100; % frequency

% graph
Fig = figure('Name', 'Non-inverting summing amplifier',...
'Units', 'normalized', ...
...
'String', A1, ...
'Callback', 'A1 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'String', A2, ...
'Callback', 'A2 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'String', R1, ...
'Callback', 'R1 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'String', R2, ...
'Callback', 'R2 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'String', R3, ...
'Callback', 'R3 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'String', R4, ...
'Callback', 'R4 = str2num(get(gco,''string'')), close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4);');
...
'string', 'RESET',...
'Callback', 'close; summing_noninverted_sin(1, 100, 2, 100, 1e3, 1e3, 1e3, 1e3);');
...
'Callback', 'close; summing_noninverted_sin(A1, f1, A2, f2, R1, R2, R3, R4)');
...
'backgroundcolor', [0.11764 0.71764 0.066], ...
'Callback', 'close; summing_noninverted_triangle(A1, f1, A2, f2, R1, R2, R3, R4)');
...
y3 = (1 + R4 / R3) * (R2 * y1 / (R1 + R2) + R1 * y2 / (R1 + R2));

subplot('position', [0.5 0.60 0.4 0.25]);
w = imread('summing_noninverted.jpg');
imshow(w, 'InitialMagnification', 150)

subplot('position', [0.05 0.20 0.4 0.25]);
w = imread('formula_summing_noninverted.jpg');
imshow(w, 'InitialMagnification', 150)
...
n = get(hObject, 'Value');
set(iplot, 'ydata', A1 * sin(2 * pi * n * t1));
f1 = n;
set(kplot, 'ydata', (1 + R4 / R3) * (R2 * A1 * sin(2 * pi * f1 * t1) / (R1 + R2) + R1 * A2 * sin(2 * pi * f2 * t2) / (R1 + R2)))
drawnow;
end

function makeplot2(hObject, ~, t2, jplot, kplot)
n = get(hObject, 'Value');
set(jplot, 'ydata', A2 * sin(2 * pi * n * t2));
f2 = n;
set(kplot, 'ydata', (1 + R4 / R3) * (R2 * A1 * sin(2 * pi * f1 * t1) / (R1 + R2) + R1 * A2 * sin(2 * pi * f2 * t2) / (R1 + R2)))
drawnow;
end
